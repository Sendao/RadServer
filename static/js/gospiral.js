...
export var colorBal = [20, 2, 4];
export var filterBal = [1, 0, 0];
    ...
    for( i=0; i<mydepth; i++ ) {
        for( j=0; j<myheight; j++ ) {
            for( k=0; k<mywidth; k++ ) {
                ...
                if( factor > 80 ) {
                    red = green = blue = ( immortals ) ? 128 : 0
                }

                red = Math.min(255, Math.max(0, fill_red + red));
                green = Math.min(255, Math.max(0, fill_green + green));
                blue = Math.min(255, Math.max(0, fill_blue + blue));
