...
function radScanClass( dv, preloadStage = false ) {
	...
	for( var i = 0; i < classNames.length; ++i ) {
		if( classNames[ i ] in radicle && typeof radicle[classNames[i]] != 'undefined' ) {
			// console.log("Instantiate class " + classNames[i]);
			let arr = radicle[ classNames[ i ] ], wasset = false;
			var x;
			for( var j in arr ) {
				if( j[ 0 ] == '#' ) {
					var x, newValue;
              ...
              if( !dv.isForZero ) {
							  wasset = true;
								let parsed1, parsed2, z, found=new Set();
								let result = ""+arr[j];
								parsed1 = x.split(";");
								...
								for( z=0; z<parsed2.length; z++ ) {
									found.add( parsed2[z].split(":")[0].trim() );
								}
								for( z=0; z<parsed1.length; z++ ) {
									let parsed3 = parsed1[z].split(":").map( (y) => y.trim() );
									if( found.has(parsed3[0]) ) continue;
									if( result != "" ) result += "; ";
									result += parsed3[0] + ": " + parsed3[1];
								}
							  dv.setAttribute( j, result );
              }
						}
					}
				}
...
function radMergeHandler( pagedata ) {
	console.log("rmergeh");
	let paradoc = document.createDocumentFragment();
	let bpos = pagedata.indexOf("<body"), epos = pagedata.indexOf("/body>");
	let body = pagedata;
	if( bpos !== -1 && epos !== -1 ) {
		body = pagedata.substring(bpos, epos+6);
	}
	paradoc.addEventListener('DOMContentLoaded', radMergeHandler2);
	paradoc.innerHTML = body;
}

function radMergeHandler2( e )
{
	console.log("rmh2");
	let doc = e.target;
	console.log("rmh2 target: ", doc);
	const collection = doc.getElementsByTagName("*");
	var eSum = null;
	for( var el of collection ) {
		if( el.hasAttribute('xTemplate') ) {
			hasCollected = true;
			if( !eSum )	eSum = document.createElement("div");
			eSum.appendChild( el );
		}
	}
	document.documentElement.appendChild(eSum);

	radSchedLoad( eSum );
}
function radPageHandler( responsedata, viewobj )
{
	...
	radMergeHandler(radViews[viewid].data);
}
...
function radViewHandler( responsedata, viewobj, requestobj )
{
	...
	console.log("rvh", divid, responsedata.substring(0,15));
	...
	if( 'origcontent' in viewdiv )
		delete viewdiv.origcontent;

	if( 'viewFrame' in viewobj && viewobj.viewFrame ) {
		viewdiv.innerHTML = '<iframe style="width: 1024px; height: 800px; resize: both; overflow: auto" id="_vf_' + randStr(18) + '" src="about:blank"></iframe>';
		let paradoc = viewdiv.children[0].contentWindow.document;
		paradoc.open();
		paradoc.write(responsedata);
		paradoc.close();
	} else {
...
function radSchedLoadFin() {
	...
	while( sched_divs.length > 0 ) {
		...
		console.log("loaded " + dc.length + " divs");
	}
	radPost();
}
...
var translate_parms = [ "xClass", "xPath", "xIgnore", "xOptions", "xCast", "xView", "xAView", 'xCmd', "xIView" ];
  ...
  if( isValid( x = getAttribute( idiv, 'xSetObj' ) ) ) {
	...
	*/
	...
	for( y = 0; y < origin_check.length; y++ ) {
		z = origin_check[y];
		if( typeof idiv[ 'o' + z ] != 'string' && typeof( x = idiv[ z ] ) == 'string' ) {
			if( x.indexOf( "#" ) != -1 ) {
				idiv[ 'o' + z ] = x;
			}
		}
...
function scaleDiv( idiv ) {
	var x, xp, xpy, y, i, found;
	let process_children = true;

	radScanClass( radNode = idiv );
...
/ *
  ...
  if( isValid( x = idiv.xParams2 ) ) {
	...
	if( isValid( x = idiv.xCast ) ) {
		...
		var targetid;
		var tdiv = idiv;
		if( isValid( targetid = idiv.xTarget ) ) {
			tdiv = document.getElementById(targetid);
		}
		try {
			blitzTemplate( tdiv, x );
		} catch ( err ) {
			console.log( x, err );
		}
		process_children = false;
	...
	if( isValid( x = idiv.xIView ) ) {
		console.log("xiview");
		idiv.xView = idiv.xIView;
		idiv.xViewFrame = true;
	}
	if( isValid( x = idiv.xView ) ) {
		let viewFrame = 'xViewFrame' in idiv ? idiv.xViewFrame : false;
		console.log('vf: ' + viewFrame);
		if( idiv.viewLoaded != x ) {
			idiv.viewLoaded = x;
			var targetid;
			if( !isValid(targetid=idiv.xTarget) ) {
				if( !('id' in idiv) )
					idiv.id = '_el_' + randStr(21);
				targetid = idiv.id;
				/*
				let targetdiv = idiv;
				if( targetid !== null ) targetdiv = document.getElement( targetid );

				clearNode(targetdiv);
				delete targetdiv.origcontent;
				*/
			}

			let viewSetup = { viewid: x, divid: targetid };
			if( viewFrame !== false ) viewSetup.viewFrame = viewFrame;

			console.log("vs",viewSetup);
			if( x in radViews ) {
				radViewHandler( radViews[ x ].data, viewSetup, null );
			} else {
				radViews[ x ] = {
					'url': radTranslate( x ),
					'data': null
				};
				HtmlRequestGet( radViews[ x ].url, '', radViewHandler, viewSetup );
			}
		}
	}
	var viewid;
	if( isValid( viewid = idiv.xMainline ) ) {
		if( idiv.viewLoaded != viewid ) {
			idiv.viewLoaded = viewid;
			if( viewid in radViews ) {
				radPageHandler( radViews[ viewid ].data, { viewid } );
			} else {
				radViews[ viewid ] = { url: radTranslate( viewid ), data: null };
				HtmlRequestGet( radViews[ viewid ].url, '', radPageHandler, {	viewid } );
			}
		}
	}
...
//		build.setAttribute( "xroot", xRoot );
	...
	if( isValid( x = idiv.xLoop ) && idiv.style.display != 'none' ) {
        ...
        */
		...
		process_children = false;
	} else if( isValid( x = idiv.xOptions ) ) { // xOptions=_a=0,b=1,c=2,d=3...     xOptions=path_to_object           xOptions=path,valuefield,descfield
		x = radTranslate( x );
		var exp_path, exp_val, exp_desc, exp_list, tform;
		let path_value = 'xVal' in idiv ? idiv.xVal : idiv.value;
		if( x.substr( 0, 1 ) == '_' ) {
		...
		if( vtype == 0 ) {
			for( i = 0; i < exp_list.length; i++ ) {
				...
				if( typeof val == 'undefined' || val == 'undefined' || val == '' ) continue;
				if( typeof srcHTML == 'undefined' || srcHTML == 'undefined' || srcHTML == '' ) continue;
