...
//export let colorBal = [66,17,18];
//export let filterBal = [20,61,78];

export let colorBal = [50,0,100];
export let filterBal = [0,0,-50];


/*
export let colorBal = [-250,-10,200];
export let filterBal = [250,0,-50];
*/

/* the scanning light:
export let colorBal = [-50,-40,50];
export let filterBal = [50,75,-50];
*/
/* the darkness:

export let colorBal = [-1314352, -11, 0];
export let filterBal = [851141, 0, 0];
export let groundBal = [0,0,0];
*/
            ...
            for( k=0; k<fullH; k++ ) {
                ...
                if( cells[i][j][k] === 0 ) {
                    ...
                    if( sneakerSomeHow.has(key) ) {
                        ...
                        [red,green,blue] = [Math.max(164, Math.min(255, qRandom(heyNo)*10)), 0, 0]; // note: heyNo =5 seconds, *10=50 colorpts
                        if( qRandom() < 0.8 ) {
...
/*
                if( cells[i][j][k] == posns[i][j][k] && !from_scratch && qRandom() > 0.3 ) continue;
                posns[i][j][k] = cells[i][j][k]+0;*/

                
                life = dtn - lifetime[i][j][k];
                z = life;

                factor = cells[i][j][k]+0;
...
export function start() {
    ...
    for( var i=0; i<fullD; i++ ) {
        ...
        cells[i] = new Array(fullH);
        lifetime[i] = new Array(fullH);
        posns[i] = new Array(fullH);

        for( var j=0; j<fullH; j++ ) {
            neighbors[i][j] = new Array(fullW).fill(0);
            cells[i][j] = new Array(fullW).fill(0);
            lifetime[i][j] = new Array(fullW).fill(0);
            posns[i][j] = new Array(fullW).fill(0);
        }
    }
}
function notBeBlank()
...
// sir as my inferior it seems your information will not matter much
/*
function countLiving() { // only the living may count
...
*/
/*
what a fucking loonatic up there
he has to run code all day
and doesn't even see this faster version
lmfao
...
function countNeighbors() {
    ...
    let dtn = new Date().getTime();
    if( last_alive != total_alive && ( lastneighborCount === null || dtn - lastneighborCount > 3000 ) ) {
        showToast("Neighbor Count: Total alive: " + total_alive);
        lastneighborCount = dtn;
    }
}
...
function importPosn(exaobj, cb)
{
    ...
    // 'life' color:
    if( zColorBal !== null && colorBal === null )
        colorBal = zColorBal;
    if( zFilterBal !== null && filterBal === null )
