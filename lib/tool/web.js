...
/* still a bit buuggy:
            if( !(whoisit in this.validRequesters) ) { // require validation first:
            ...
            if (req.method == 'POST') {
                ...
                try {
                    ...
                    this.app.log('error', "Busboy error request headers: " + JSON.stringify(copyof));
                    this.app.log('error', "Busboy error: " + JSON.stringify(err) );
                    this.app.log('error', err);

                    //this.app.util.throwStack('Busboy');
    ...
    WebObject.prototype.initialize = async function () {
        this.app.log('web', 
            "http starting...");
        this.server = await http.createServer(this.mainHandler_Insecure.bind(this));
        if ('ssl' in this.app.config ) {//&& await fs.existsSync(this.app.config['ssl'])) {
            this.app.log('web', "https starting...");
            this.https_server = await https.createServer({ pfx: fs.readFileSync(this.app.config['ssl']) }, this.mainHandler.bind(this));
        } else {
        ...
        if( await fs.existsSync('bannedips.txt') ) {
            try {
                this.bannedIps = JSON.parse( fs.readFileSync('bannedips.txt') );
            } catch( e ) {
                console.log(e);
                this.bannedIps = {};
            }
        ...
        if( await fs.existsSync('validips.txt') ) {
            try {
                this.validRequesters = JSON.parse( fs.readFileSync('validips.txt') );
            } catch( e ) {
                console.log(e);
                this.validRequesters = {};
            }
    ...
    WebObject.prototype.startup = function () {
        this.app.log('debug', "ws starting...");
        ...
        if ('ssl' in this.app.config && fs.existsSync(this.app.config['ssl'])) {
            this.app.log('debug', "wss starting...");
        ...
        if( this.wssserver ) {
            this.wssclient = new this.app.tools.Websock.client("wss://" + this.app.config['hostname'] + ":" + this.using_ssl_port + "/", "echo-protocol");
        }
    ...
    WebObject.prototype.checkUrl = function( url, ipaddr ) {
        let passed = true;
    ...
    WebObject.prototype.saveBans = function() {
        try {
 ...
 2 + 3-1=4,  -->1 ... 3 // (4-1=3) // hey hey what's with the three second pause h
  4 + 4-2=6,  -->0 ... 3 // (6-3=3)
    ...
    */
    ...
    WebObject.prototype.closeLoadHandler = function (ipaddr, loaderId) {
...
*/
	...
	WebObject.prototype.autorouter = async function( req, res, params ) {
