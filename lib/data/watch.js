...
module.exports = function WatchData() {
    ...
    this.loadMonitorConfig = function()
    {
        fs.readFile('/var/rad/monitor.cfg', 'utf8', function(err, data) {
            ...
            this.saveConfig(cfgdata);
        });
    };
    
    this.saveConfig = function( cfgdata )
    {
    	this.configData = cfgdata;
        console.log('config data loaded', cfgdata);
        // cfgdata[groups] = c_group
        // c_group = { name, processes=[c_runprocs], logfiles=[c_logfiles], requires=[c_requires] }
        // c_runproc = { name, mainpid, cwd, env, psgrep, runstate, cmdstate, lastdown, startat, pids=[c_pid] }
        // c_pid = { pid, cpu, cpu2, cpu3, cmdline }
        // c_logfile = { path, isdir: 0|1 }
        // c_require = "string"
    };
    ...
    this.loadMonitorStatus = function()
    {
    	fs.readFile('/tmp/monitor.status', 'utf8', function(err, data) {
            ...
            for( i=0; i< parser.length; i++ ) {
                ...
                switch( cmd[0] ) {
                    case 'group':
                    	c_group = this.findGroup(val);
                        c_mode = 1;
                        break;
                    case 'process':
                    	c_rp = this.findProcess(c_group, val);
                        c_mode = 2;
                        break;
                    case 'logfile':
                    	if( c_mode == 3 || c_mode == 4 )
                    	...
                    	if( c_mode == 1 ) {
                            c_lf = this.findLogfile(c_group, val);
                            lastmode = c_mode;
                            c_mode = 3;
                    	} else {
                            c_lf = this.findLogfile(c_rp, val);
                            lastmode = c_mode;
                            c_mode = 3;
                    	}
                        break;
                    case 'watch':
                    	...
                    	if( c_mode == 1 ) {
                    		c_watch = this.findWatch(c_group, val);
                    		lastmode = c_mode;
                    		c_mode = 4;
                    	}
                    	break;
