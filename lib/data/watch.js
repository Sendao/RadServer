    ...
    this.loadMonitorConfig = function()
    {
        fs.readFile('/tmp/monitor.status', 'utf8', function(err, data) {
            ...
            for( i=0; i< parser.length; i++ ) {
                ...
                switch( cmd[0] ) {
                    case 'group':
                        c_group = { 'name': parser2.join(' '), 'state': 0, 'processes': [], 'logfiles': [], 'logto': '', 'logcount': 0, 'logsize': 0, 'requires': [] };
                    ...
                    case 'require':
                        ...
                        break;
                    case 'process':
                        c_rp = { 'name': parser2.join(' '),
                            'mainpid': -1,
                            'cwd': '', 'env': '', 'psgrep': '',
                            'runstate': 0, 'cmdstate': 0,
                            'lastdown': 0, 'startat': 0,
                            'pids': [] };
