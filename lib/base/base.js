...
module.exports = function DatabaseApp() {
    ...
    this.convey = function(dbname) {
        ...
        this.search2 = function( qry, page )
        {
            var fids = this.find2(qry, page);
            var results = this.retrieve(fids);
            return results;
        };
...
//T LOOK AT ME!!!!
        ...
        this.find2 = function( qry, page ) {
            if( typeof qry == 'string' ) {
                ...
                var param = vals.pop();
                var value = vals.join('=');
                return this.find(param,value,page['start'],page['count']);
            } else {
                var vals = [];
                var tgts = [];
                for( var i in qry ) {
                ...
                return this.find(tgts,vals,page['start'],page['count']);
            }
        };
    ...
    /* LEVEL 0: disk-oriented construction */
    ...
    this.registerClient  = function( base, client, code, cond ) {
        ...
        base.clients.splice(i+1,0,[ code, cond ]);
    };
    ...
    this.sendMessage = function( base, code, modified_obj ) {
        ...
        for( i = 0; i < len; ++i ) {
            for( j=i+1; j<len; ++j ) {
                if( cc.app.util.typeOf(base.clients[i]) != 'array' )
                    break;
                if( base.clients[j][0] == false ) {
                    base.messages.push( [ base.clients[i], code, modified_obj ] );
                } else if( code == base.clients[j][0] ) {
                    cond = base.clients[j][1];
                    if( cond === true || base.testCondition( modified_obj[cond[0]], cond[1], cond[2] ) ) {
                        base.messages.push( [ base.clients[i], code, modified_obj ] );
                    }
                }
            }
...
//    	if( 'workcycle' in base && typeof base.workcycle == 'function' ) {
 //   	    base.workcycle();
   // 	}
    	...
    	if( base.dirtyindex != 0 ) {
    		this.workTime = 0;
    		console.log("Work queue", base.name);
   			this.clean(base);
   			base.queue = [];
   			base.dirtyindex = 0;
    		return true;
...
//    		console.log("Skip queue", base.name, base.workTimer, timeNow);
    ...
    /* quietWorker: slow down the processing */
...
//    	console.log("db " + base.name + " clean");
    ...
    /* indexDirty: flag the index for rewrite */
