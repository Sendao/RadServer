...
{ // 'convey'
    ...
    // usage: registerClient( app.locateUser( key ), code, [testOf, opcode, value] );
    // usage: sendMessage( code, modifiedObj ); 
    this.registerClient = function( client, code, cond ) {
        this.db.registerClient( this, client, code, cond );
    };
    this.sendMessage = function( code, modifiedObj ) {
        this.db.sendMessage( this, code, modifiedObj );
    };
    ...
    // save: saves a new record directly to file, or updates a record
    this.save = function( new_record ) {
        ...
        var handles = [];
        var handle = this.getHandle(new_record);
        if( handle !== false ) {
            var objs = {};
            objs[handle] = new_record;
            console.log("UpdHandle");
            handles = this.db.updRecords( this, objs );
        } else {
    ...
    // saveBack: queue an item to be saved asynchronously
    this.saveBack = function( new_record ) {
        var isnew = true;
        ...
        if( handle !== false ) {
            var objs = {};
            objs[handle] = new_record;
            console.log("UpdHandleBack");
            handles = this.db.updRecordsLater( this, objs );
        } else {
            console.log("NewHandleBack");
            handles = this.db.addRecordsLater( this, [ new_record ] );
        }
        console.log("SaveHandlesBack(", handles, ")");
        this.db.saveHandlesBack( this, handles );
        if( this.use_messaging )
            this.db.sendMessage( this, this.name + "_update", new_record );
    };
    ...
    this.searchrange = function( qry, range )
    {
        var fids = this.findrange(qry, range);
        var results = this.retrieve(fids);
        return results;
    };
    this.searchlist = function( qry, start, count )
    {
        var fids = this.findrange(qry, { 'start': start, 'count': count } );
        var results = this.retrieve(fids);
        return results;
    };
    ...
    this.edit = function(handle, obj, autoClear=false) {
        ...
        if( rec == false || rec == null ) {
            ...
            handle = this.getHandle( rec );
            if( handle === false ) {
                console.info("Invalid handle for record ", rec);
                return false;
            }
            handles = this.db.updRecords( this, handles );
            handle = handles[0];
        }
    ...
    this.store = this.write = function( item ) {
        ...
        var hx;
        hx = this.db.saveScriptForHandle(idx); 
        if( this.use_messaging )
    ...
    this.find = function( by, val ) {
        ...
        var bye;
        //console.log(this.name, "find("+by+","+val+")");
        if( typeof by == 'string' ) {
            var ind = by;
            by = {};
            by[ind] = val;
        ...
        for( i in by ) {
            ...
            if( app.util.typeOf(value) == 'array' ) {
                ...
                //console.log("found(" + value + ") in indice[" + bye + "]: " + idx.length);
                for( j=0; j<idx.length; ++j ) {
                    results.push(idx[j]);
                    //console.log("entry(", idx[j], ")");
                }
            } else {
                results = false;
            }
        }
        //console.log("fill: " + results.length);

        return results;
    };
    this.findrange = function( qry, page ) {
//        console.log("find2("+qry+","+page+")");
    ...
    this.findlist = function( by, value, start, count ) {
        var i, results=[], j, idx, len;
        var bye;
        if( typeof by == 'string' ) {
            by = { by: value };
        }
        console.log("findlist(",by,value,start,count,")");
