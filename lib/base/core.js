 ...
 * 
 * Todo:
 ** Why do records duplicate
 ** Allow for saveIndexOf single records
 * - Add indexPos, indexLen to index
 * - calculate indexLen 3 times
 * - add 5-char buffer
 *
 */
...
{ // 'base'
    ...
    this.clearSpace = function( base, idx, bytes ) {
    	...
    	base.fileindex.push( [ idx, bytes, 0, 0, 5 ] );
    ...
    this.loadScript = function( base ) {
        ...
        console.info("loadScript(" + base.name + ")");
        this.cleanIndex(base);
        base.fileindex = [];
        this.buildDirFor(fn);
        if( !fs.existsSync(fn) ) {
