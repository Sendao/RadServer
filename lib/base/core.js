...
//        console.log("db", dbname);
  ...
  //      if( base.name == 'marky' )
//            console.info("workcycle(marky)");
        
        for( i in base.tables ) {
    //        if( base.tables[i].name == 'words' ) {
      //          console.info("bg(words)");
        //    }
 ...
 //           console.log("db." + base.name + " sending messages " + base.messages.length);
    ...
    this.subscribe = this.registerClient  = function( base, client, code, cond ) {
        if( !('clients' in base) || typeof base.clients == 'undefined' )
            base.clients = [];
        var i, len = base.clients.length;
        if( typeof code == 'undefined' )
            code = false;
        for( i=0; i<len; ++i ) {
            if( base.clients[i][0] == client && ( code == false || base.clients[i][1] == code ) ) {
                // already subscribed to this code
                base.clients.splice(i, 1, [ client, code, cond ] );
                return;
            } 
        }
        base.clients.push( client, code, cond );
    };
    
    /* Clean client list. Client list is used for messaging. */ 
    this.clearClient = function( base, client, code ) {
        if( typeof code == 'undefined' ) code = false;
        var i = 0, len = base.clients.length;
        if( typeof code == 'undefined' )
            code = false;
        for( i=0; i<len; ++i ) {
            if( base.clients[i][0] == client && ( code == false || base.clients[i][1] == code ) ) {
                // unsubscribe this code
    ...
    this.sendMessage = function( base, code, modified_obj ) {
        var i, j, cond, passedAll;
        //console.log("Broadcast: [" + code + "]", typeof modified_obj);
        
        i = base.clients.length;// clients = [ client, code, conds=[(index,opcode,value),cond2,cond3,...], client2, code2,... ]
        while( i > 0 ) {
            i-=3;
            if( base.clients[i][0] == client && ( code == false || base.clients[i][1] == code ) ) 
            {
                cond = base.clients[i][2];
                passedAll = true;
                j = cond.length;
                while( j > 0 ) {
                    j -= 3;
                    if( !this.testCondition( modified_obj[cond[j+0]], cond[j+1], cond[j+2] ) ) {
                        passedAll = false;
                        break;
                    }
                }
    ...
    /* _sendMessages: sends updates via websockets */
...
//                    console.log("buffer data", bufstr);
                ...
                if( !isNaN(fidx[3]) ) {
                    ...
                    this.constructIndexFor( base, fidno, true, false ); // indices[]
                    fidx[4] = 1;
                }
            }
    ...
    this.indexFromRecords = function(base) {
        ...
        for( j=0; j<base.records.length; j++ ) {
            ...
            this.constructIndexFor(base, j, false);
        }
    ...
    this.constructIndexFor = function(base, fidno, dontSave)
    {
        var fi = base.fileindex[fidno][2];
        var i;
        
        //console.info("ConstructIndexFor("+base.name+","+fidno+","+(dontSave?"nosave":"+save")+")");
        for( i in base.indice ) {
